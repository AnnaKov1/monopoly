...
interface Property extends Tile {
    price: number;
    rent: number[];
    houseCost: number;
    hotelCost: number;
...
interface Utility extends Tile {
    price: number;
    rentMultiplier: number[];
    owner?: Player;
    type: 'utility';
...
interface Railroad extends Tile {
    price: number;
    rent: number[];
    owner?: Player;
    type: 'railroad';
...
interface Chance extends Tile {
    type: "chance";
}

interface CommunityChest extends Tile {
    type: "communityChest";
...
const tiles: Tile[] = tilesData.map(tile => ({
    ...tile,
    element: document.getElementById(tile.id)!
}));
...
const movePlayerIcons = () => {
    players.forEach((player, index) => {
        ...
        const existingIcon = document.querySelector(`.player${index + 1}-icon`) as HTMLElement;
        ...
        currentTile?.element.appendChild(playerIcon);
    });
};
...
document.addEventListener("DOMContentLoaded", () => {
    const buyHouseButtons = document.querySelectorAll(".buy-house") as NodeListOf<HTMLButtonElement>;
    const buyRailroadButtons = document.querySelectorAll(".buy-railroad") as NodeListOf<HTMLButtonElement>;
    const buyUtilityButtons = document.querySelectorAll(".buy-utility") as NodeListOf<HTMLButtonElement>;

    buyHouseButtons.forEach((button, index) => {
        button.addEventListener('click', () => {
            const currentPlayer:Player = players[index];
            const currentTile = tiles[currentPlayer.position];

            if (currentTile && currentTile.type === "property" && currentPlayer.money >= (currentTile as Property).houseCost) {
                handleBuyHouse(currentPlayer, currentTile as Property);
    ...
    buyRailroadButtons.forEach((button, index) => {
        button.addEventListener('click', () => {
            ...
            if (currentTile && currentTile.type === "railroad" && currentPlayer.money >= (currentTile as Railroad).price) {
                handleBuyRailroad(currentPlayer, currentTile as Railroad);
    ...
    buyUtilityButtons.forEach((button, index) => {
        button.addEventListener('click', () => {
            ...
            if (currentTile && currentTile.type === "utility" && currentPlayer.money >= (currentTile as Utility).price) {
                handleBuyUtility(currentPlayer, currentTile as Utility);
...
const handleBuyHouse = (currentPlayer: Player, property: Property): void => {
...
const handleBuyRailroad = (currentPlayer: Player, railroad: Railroad): void => {
...
const handleBuyUtility = (currentPlayer: Player, utility: Utility): void => {
...
const handleProperty = (currentPlayer: Player, tile: Property | Railroad | Utility): void => {
    if (!tile.owner) {
...
const updatePlayerProperties = (player: Player) => {
    const playerPropertiesDiv = document.querySelector(`.player${players.indexOf(player) + 1} .player-properties`) as HTMLDivElement;
...
const handleRentPayment = (currentPlayer: Player, tile: Property | Railroad | Utility): void => {
    const owner = tile.owner;

    if (owner && owner !== currentPlayer) {
        let rentArray: number[] | undefined;
        ...
        if (currentPlayer.money >= rentAmount) {
            ...
            alert(
                `${currentPlayer.name} does not have enough money to pay $${rentAmount} in rent to ${owner.name}. 
...
const handleChanceOrCommunityChest = (currentPlayer: Player, tile: Chance | CommunityChest): void => {
    //@ts-ignore
...
const handleTileAction = (currentPlayer: Player, currentTile: Tile): void => {
    switch (currentTile.type) {
        case "property":
        case "railroad":
        case "utility":
            if (!currentTile.owner) {
                handleProperty(currentPlayer, currentTile as Property);
            } else {
                handleRentPayment(currentPlayer, currentTile as Property | Railroad | Utility);
            }
            break;
        case "special":
        ...
        case "chance":
        case "communityChest":
            handleChanceOrCommunityChest(currentPlayer, currentTile as Chance | CommunityChest);
            break;
    }
};
...
document.getElementById("rollDiceButton")?.addEventListener('click', startTurn);
